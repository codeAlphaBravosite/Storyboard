...
// Enhance preview manager to maintain the current storyboard context
const originalRenderPreview = previewManager.renderPreview;
previewManager.renderPreview = function(storyboard) {
    // Store the current storyboard being previewed
    this.currentStoryboard = storyboard;
    
    // Call the original render preview method
    originalRenderPreview.call(this, storyboard);
    
    // Setup edit button to load the specific storyboard
    const editBtn = document.getElementById('backToEditBtn');
    if (editBtn) {
        const newEditBtn = editBtn.cloneNode(true);
        editBtn.parentNode.replaceChild(newEditBtn, editBtn);
        ...
        newEditBtn.onclick = () => {
            ...
            // Ensure preview button works for this specific storyboard
            const previewFromEditorBtn = document.getElementById('previewFromEditor');
            if (previewFromEditorBtn) {
                previewFromEditorBtn.onclick = () => {
                    previewManager.renderPreview(storyboard);
    ...
    // Setup export for this specific storyboard
    setupExportButton(storyboard);
};
